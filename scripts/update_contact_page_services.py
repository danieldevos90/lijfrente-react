#!/usr/bin/env python3
"""
Update contact page services to include only phone and email (remove visit option)

USAGE:
    export STRAPI_API_TOKEN='your-token-here'
    python3 scripts/update_contact_page_services.py
"""
...
def update_contact_services():
    """Update contact page services section with phone and email only"""
    print("\n📞 Updating contact page services...")
    ...
    # Update sections - update services-section with phone and email
    updated_sections = []
    for section in clean_sections:
        if isinstance(section, dict):
            # Check if this is a services-section for contact
            if section.get('__component') == 'sections.services-section' and (
                ...
                # Update with phone and email services only
    ...
    try:
        ...
        if response.status_code == 200:
            print("✅ Successfully updated contact page services")
            print("   - Bel ons (085-0480881)")
            print("   - E-mail ons (info@geldgeregeld.nl)")
            return True
        else:
...
def main():
    print("🔄 Updating contact page services...")
    print(f"📍 Site ID: {SITE_ID}")
    print(f"🔗 Strapi URL: {STRAPI_URL}\n")
    
    success = update_contact_services()
